...
const usedPlayersList = document.getElementById("usedPlayersList");
...
// --- Fetch appearances from Wikipedia ---
...
// --- Update Used Players List ---
function updateUsedPlayersList() {
  usedPlayersList.innerHTML = "";
  usedPlayers.forEach(p => {
    const li = document.createElement("li");
    li.textContent = p;
    usedPlayersList.appendChild(li);
  });
}
...
throwBtn.addEventListener("click", async () => {
  ...
  if (apps === null) {
    const manual = prompt(`Could not fetch appearances for ${playerName} at ${team}. Enter manually:`);
